...
export default class DevToolUtils {
    ...
    public static setupObj(obj:any, folderName:string, rootFolder:GUI, ignore?:string[], onAnyUpdate?:() => void):GUI {
        ignore = ignore ?? [];
        const folder = folderName ? rootFolder.addFolder(folderName) : rootFolder;
        Object.getOwnPropertyNames(obj)
            .filter(methodName => ignore!.indexOf(methodName) == -1)
            .filter(methodName => methodName[0] != "_")
            .forEach(methodName => {
                if (typeof obj[methodName] == "object") {
