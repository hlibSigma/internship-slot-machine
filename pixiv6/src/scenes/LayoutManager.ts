...
type AspectLayoutKey = `${number}/${number}`;
...
export default class LayoutManager {
    ...
    private getLayoutFromPartial(partialLayout:Partial<PartialLayout> & Required<LayoutName>):Layout {
        if (partialLayout.extend && partialLayout.extend != "") {
            this.uniqueLayouts.filter(value => value.uid == partialLayout.extend || value.uid && partialLayout.extend!.indexOf(value.uid) >= 0).forEach(baseLayout => {
    ...
    update(gameSize:LayoutGameSize) {
        ...
        console.log("Layout update: ", Date.now() - time);
    }
    ...
    private calculateLayout(layoutSize:LayoutSize, layout:Layout):Layout {
        ...
        switch (layout.display) {
            ...
            case "relative": {
                ...
                layout.layouts.forEach((value, i, arr) => {
                    ...
                    finalLayout.width -= sortByHorizontal ? value.finalLayout.width : 0;
                    finalLayout.height -= sortByVertical ? value.finalLayout.height - (childLayoutSize.y - value.finalLayout.y) : 0;
    ...
    private calculateLayoutTopPosition(layout:Layout, layoutSize:LayoutSize) {
        let layoutTop = this.getLayoutTopPosition(layout, layoutSize);
        switch (layout.align) {
            case "bl":
            case "br":
            case "b":
            ...
            case "l":
            case "c":
                layout.finalLayout.y = layoutSize.y + (layoutSize.height - layout.finalLayout.height) / 2 + layoutTop /*+ layoutTop*/;
                break;
        }
    }

    private calculateLayoutLeftPosition(layout:Layout, layoutSize:LayoutSize) {
        let layoutLeft = this.getLayoutLeftPosition(layout, layoutSize);
        switch (layout.align) {
            case "tr":
            case "br":
            case "r":
    ...
    // noinspection JSMethodCanBeStatic and SuspiciousTypeOfGuard
    private getLayoutTopPosition(layout:Layout, layoutSize:LayoutSize) {
        let result:number = 0;
    ...
    private getLayoutLeftPosition(layout:Layout & LayoutName & Layouts, layoutSize:LayoutSize) {
        let result:number = 0;
    ...
    protected overrideLayout(partialLayout:Layout, baseLayout:Partial<PartialLayout>, clone:boolean = true):Layout {
        let result:Layout;
        if (clone) {
